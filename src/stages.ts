...
const STAGE_1_TEMPLATES: ChunkTemplate[] = [
  ...
  { patternId: 's1_raw', raw: {
    ...
    grounds: [
      // ─── ベース: 上下段とも住宅タイル (Act I residential 統一) ───
      _G('residential_tile', 0, 46.5, 360, 93),
      _G('residential_tile', 0, 153.5, 360, 93),
      ...
      _G('concrete', 38, 58, 30, 26),                           // ランドロマット前
      ...
      _G('concrete', -130, 135, 36, 28),                        // マンション玄関 (institutional tile 撤去)
      _G('wood_deck', -48, 170, 26, 18),                        // Cell F 老人縁側 (盆栽の下)
      _G('grass', 35, 170, 50, 32),                             // Cell G 住宅の裏庭
  ...
  { patternId: 's1_raw', raw: {
    ...
    grounds: [
      _G('residential_tile', 0, 46.5, 360, 93),              // Act II 序盤はまだ住宅が混じる
      _G('concrete', 0, 153.5, 360, 93),
      _G('concrete', -45, 55, 30, 45),                       // コンビニ前の舗装
      _G('concrete', 35, 55, 25, 40),                        // クリーニング前
      // ─── ★ C3 固有: 3 連テラスの wood_deck (カフェ/本屋/パン屋) ───
      _G('wood_deck', -15, 140, 100, 50),                    // ★ テラス広場 (木デッキで統一)
  ...
  { patternId: 's1_raw', raw: {
    ...
    grounds: [
      ...
      _G('concrete', 0, 153.5, 360, 93),
      _G('wood_deck', -125, 55, 30, 40),                     // パン屋前 (暖色木デッキ)
      _G('wood_deck', -55, 55, 30, 45),                      // 本屋前 (暖色木デッキ)
      _G('concrete', 50, 55, 50, 50),                        // スーパー前 (汎用コンクリ)
      _G('tile', 152, 55, 25, 42),                           // 薬局前 (半公共)
      _G('wood_deck', -55, 135, 30, 40),                     // カフェ前 (暖色木デッキ)
      _G('concrete', 40, 135, 25, 40),                       // クリーニング前
      _G('tile', 92, 165, 24, 32),                           // 銭湯前 (公共)
  ...
  { patternId: 's1_raw', raw: {
    ...
    grounds: [
      ...
      _G('concrete', 0, 153.5, 360, 93),
      _G('wood_deck', -125, 55, 35, 45),                     // カフェ前 (暖色木デッキ)
      _G('tile', -50, 55, 40, 45),                           // 郵便局前 (公共)
      _G('tile', 45, 55, 40, 45),                            // 銀行前 (公共)
      // ─── ★ C5 固有: 地域広場の中央円 ───
      _G('stone_pavement', 0, 78, 60, 36),                   // ★ 広場の中央 (石畳で格式を持たせる)
      _G('wood_deck', 150, 55, 28, 40),                      // パン屋前 (暖色木デッキ)
      _G('grass', -130, 135, 30, 40),                        // 花屋前 (フラワーショップは緑地)
      _G('wood_deck', -55, 135, 60, 70),                     // 本屋+カフェ+薬局 小商店クラスタ (暖色木デッキ)
      _G('concrete', 40, 135, 28, 40),                       // クリーニング前
  ...
  { patternId: 's1_raw', raw: {
    ...
    grounds: [
      ...
      // 下段商店街 (小学校帰りの道、暖色と公共の混在)
      _G('wood_deck', -55, 135, 30, 45),                     // カフェ/パン屋 (暖色)
      _G('concrete', -20, 135, 25, 45),                      // ラーメン/居酒屋 (汎用)
      _G('wood_deck', 35, 135, 30, 45),                      // 本屋/カフェ (暖色)
      _G('concrete', 68, 135, 22, 45),                       // 小商店 (汎用)
      _G('concrete', -55, 180, 30, 25),                      // 裏路地
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      // ─── Act III 継続桜: 商店街の街路樹 (C6 からの連続) ───
      _F('sakura_tree', -170, 92),                           // 左端の桜
      _F('sakura_tree', 172, 92),                            // 右端の桜
      _F('sakura_tree', -170, 185),                          // 下段左端
    ],
    humans: [
    ...
    grounds: [
      _G('concrete', 0, 46.5, 360, 93),
      _G('concrete', 0, 153.5, 360, 93),
      _G('wood_deck', -158, 55, 28, 45),                     // 本屋前 (暖色)
      _G('wood_deck', -125, 55, 25, 45),                     // カフェ前 (暖色)
      _G('wood_deck', -60, 55, 30, 45),                      // パン屋前 (暖色)
      _G('grass', -25, 55, 25, 45),                          // 花屋前 (緑地)
      _G('tile', 35, 55, 30, 45),                            // 薬局前 (半公共)
      _G('wood_deck', 70, 55, 25, 45),                       // 本屋前 (暖色)
      _G('concrete', 115, 55, 30, 45),                       // クリーニング前
      _G('concrete', -158, 148, 28, 42),                     // ラーメン前
      _G('concrete', -55, 148, 28, 42),                      // 居酒屋前
      _G('wood_deck', -20, 148, 25, 42),                     // カフェ前 (暖色)
      _G('wood_deck', 35, 148, 25, 42),                      // 本屋前 (暖色)
      _G('wood_deck', 68, 148, 25, 42),                      // パン屋前 (暖色)
      _G('concrete', 0, 15, 120, 25), _G('concrete', 0, 188, 120, 20),
    ],
    horizontalRoads: [_MID_HR], verticalRoads: [..._SPINE_V],
  } },
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      // ─── Act III 終端の桜 (駅前の並木、Stage 1 最後の花) ───
      _F('sakura_tree', -155, 90),                           // 駅前広場の桜
      _F('sakura_tree', 155, 90),                            // 駅前広場の桜
      _F('sakura_tree', -150, 185),                          // 下段商店列の桜
    ],
    humans: [
    ...
    grounds: [
      _G('concrete', 0, 46.5, 360, 93),
      _G('concrete', 0, 153.5, 360, 93),
      _G('asphalt', -40, 72, 90, 40),                        // 駅前広場 (駅前は正当)
      _G('stone_pavement', -10, 60, 30, 30),                 // 駅前石畳
      _G('concrete', 50, 55, 30, 40),                        // コンビニ前
      _G('tile', 88, 55, 28, 40),                            // 郵便局前 (公共)
      _G('wood_deck', 178, 55, 20, 40),                      // カフェ前 (暖色)
      _G('wood_deck', -55, 135, 30, 45),                     // カフェ前 (暖色)
      _G('wood_deck', -20, 135, 25, 45),                     // 本屋前 (暖色)
      _G('wood_deck', 35, 135, 30, 45),                      // パン屋前 (暖色)
      _G('grass', 68, 135, 22, 45),                          // 花屋前 (緑地)
      _G('concrete', 0, 15, 120, 25), _G('concrete', 0, 188, 120, 20),
    ],
    horizontalRoads: [_MID_HR], verticalRoads: [..._SPINE_V],
  } },
