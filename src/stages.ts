...
const STAGE_1_TEMPLATES: ChunkTemplate[] = [
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      // 蔵 (kura) 帰属: 古い農具の蔵
      _F('milk_crate_stack', -135, 22), _F('wood_fence', -160, 50),
      // 茶屋 (chaya) 帰属: 暖簾と提灯
      _F('noren', -30, 64), _F('chouchin', -30, 62),
      _F('bench', -45, 92), _F('potted_plant', -15, 78),
      ...
      _F('hedge', 53, 30), _F('hedge', 53, 50), _F('hedge', 53, 70), _F('hedge', 53, 88),
      // ロット2境界: x=113 で house と mansion を分ける
      _F('wood_fence', 113, 30), _F('wood_fence', 113, 50), _F('wood_fence', 113, 70), _F('wood_fence', 113, 88),
      // ロット3境界: x=167 で mansion と east house を分ける
      _F('hedge', 167, 30), _F('hedge', 167, 50), _F('hedge', 167, 70), _F('hedge', 167, 88),
      ...
      _F('wood_fence', -118, 130), _F('wood_fence', -118, 145), _F('wood_fence', -118, 162), _F('wood_fence', -118, 178),
      // ロット境界: x=-65 (house と townhouse) — 現代 hedge
      _F('hedge', -65, 130), _F('hedge', -65, 145), _F('hedge', -65, 162), _F('hedge', -65, 178),
      ...
      _F('hedge', 60, 130), _F('hedge', 60, 145), _F('hedge', 60, 162), _F('hedge', 60, 178),
      // ロット境界: x=115 (duplex と mansion)
      _F('wood_fence', 115, 130), _F('wood_fence', 115, 145), _F('wood_fence', 115, 162), _F('wood_fence', 115, 178),
      ...
      _F('flower_planter_row', 80, 188),
      // SE 裏路地家具
      _F('garbage', 30, 168),                                     // shed 帰属
      _F('flower_bed', 120, 195),                                  // greenhouse 横
      _F('milk_crate_stack', 120, 168), _F('bicycle_rack', 165, 168),  // garage 帰属
      // ═══ 連続軸: 桜並木 (avenue 沿いに点在、Ch0-Ch5) ═══
      _F('sakura_tree', -135, 88), _F('sakura_tree', 30, 88),
      _F('sakura_tree', 165, 88),
      _F('sakura_tree', -22, 178), _F('sakura_tree', 130, 178),
      // ═══ 連続軸: 電柱+電線 (奥層 4 隅) ═══
    ...
    humans: [
      ...
      _H(-30, 90),
      // NE 現代住宅 (4): 各家の玄関前
      _H(22, 50),     // townhouse(30, 30)
      _H(70, 50),     // house(80, 30)
      _H(130, 50),    // mansion(145, 30)
      _H(165, 80),    // house(175, 60)
      // SW 下段住宅 (3): 各家の玄関前
      _H(-150, 152),  // machiya
      _H(-90, 152),   // house
      _H(-45, 152),   // townhouse
    ...
    grounds: [
      ...
      // ═══ NE 住宅列 (4 ロット、各家の敷地を地面で表現) ═══
      // ロット1: townhouse(30, 30) 「花好きの家」→ grass 庭の敷地
      _G('grass', 30, 56, 46, 72),
      // ロット2: house(80, 30) 「禅の庭の家」→ tile 玄関+石庭
      _G('tile', 83, 56, 60, 72),
      // ロット3: mansion(145, 30) 「邸宅」→ concrete 大駐車+grass庭
      _G('concrete', 139, 40, 52, 40),
      _G('grass', 139, 80, 52, 28),
      // ロット4: house(175, 60) 「家庭菜園の家」→ dirt 畝
      _G('dirt', 173, 56, 14, 72),

      // ═══ SW 住宅列 (3 ロット) ═══
      // ロット1: machiya(-150, 130) 「伝統の家」→ grass + dirt 石庭
      _G('grass', -148, 152, 60, 40),
      _G('dirt', -135, 175, 24, 14),  // 石庭の土
      // ロット2: house(-90, 130) 「子育て家庭」→ tile 玄関+花壇
      _G('tile', -90, 152, 53, 40),
      // ロット3: townhouse(-45, 130) 「若夫婦」→ concrete 駐車
      _G('concrete', -38, 152, 55, 40),

      // ═══ SE 住宅列 (3 ロット) ═══
      // ロット1: house(35, 130) 「ガーデニング」→ grass 花壇敷地
      _G('grass', 35, 152, 50, 40),
      // ロット2: duplex(85, 130) 「二世帯」→ concrete 大駐車+grass共用裏庭
      _G('concrete', 88, 145, 55, 30),
      _G('grass', 88, 175, 55, 14),
      // ロット3: mansion(145, 130) 「大邸宅」→ concrete 駐車+grass 噴水庭
      _G('concrete', 145, 145, 65, 30),
      _G('grass', 145, 175, 65, 14),

      // ═══ 裏路地: 1 本の concrete 帯で統一 (dy=192 全幅) ═══
  ...
  { patternId: 's1_raw', raw: ((): RawChunkBody => {
    ...
    out.grounds = [
      ...
      // ═══ 焦点ゾーン: 児童公園 (SE 全体を 1 枚のタイル広場で包む) ═══
      _G('tile', 90, 150, 180, 100),
      _G('grass', 92, 175, 160, 42),  // 公園の緑地
      _G('dirt', 90, 175, 30, 22),    // 砂場の土
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('traffic_light', -90, 92), _F('traffic_light', 87, 92),
      _F('guardrail_short', -30, 100), _F('guardrail_short', 27, 100),
      _F('bollard', -60, 92), _F('bollard', 57, 92),
      // ── 下段 住宅 facade ──
      _F('mailbox', -170, 120), _F('mailbox', -45, 122), _F('mailbox', 60, 122), _F('mailbox', 145, 120),
      _F('mailbox', -100, 122), _F('mailbox', 97, 122),
      _F('ac_unit', -100, 148), _F('ac_unit', 97, 148), _F('ac_unit', -170, 158), _F('ac_unit', 145, 158),
      ...
      _F('traffic_light', -90, 192), _F('traffic_light', 87, 192),
      _F('street_lamp', 0, 195),
      _F('guardrail_short', -55, 198), _F('guardrail_short', 52, 198),
      _F('manhole_cover', -30, 198), _F('manhole_cover', 27, 198),
      // ── タイトパッキング: 中間 dy 帯と裏路地 ──
      // dy=50 帯 (公共施設前のタイル境界)
      _F('hedge', -150, 50), _F('hedge', 165, 50),
      _F('bush', -130, 60), _F('bush', 75, 60), _F('bush', 127, 60),
      _F('flower_bed', -75, 60), _F('flower_bed', 72, 60),
      _F('potted_plant', -150, 30), _F('potted_plant', 165, 30),
      // dy=110-120 中間帯
      _F('hedge', -150, 115), _F('hedge', 147, 115), _F('hedge', -45, 115), _F('hedge', 42, 115),
      _F('bush', -100, 118), _F('bush', 0, 118), _F('bush', 97, 118),
      _F('manhole_cover', -100, 108), _F('manhole_cover', 97, 108),
      _F('bollard', -65, 92), _F('bollard', 70, 92),
      // dy=160-175 中間帯
      _F('hedge', -130, 165), _F('hedge', 127, 165), _F('hedge', -75, 165),
      _F('bush', 0, 168), _F('bush', 100, 168),
      _F('flower_bed', -150, 175), _F('flower_bed', 147, 175),
      _F('flower_planter_row', -45, 175), _F('flower_planter_row', 42, 175),
      // 裏路地家具
      _F('garbage', -150, 88), _F('garbage', 147, 88),
      _F('garbage', -75, 188), _F('garbage', 72, 188),
      _F('recycling_bin', -130, 88), _F('recycling_bin', 127, 88),
      _F('dumpster', 0, 88), _F('dumpster', -45, 188),
      _F('ac_outdoor_cluster', -100, 92), _F('ac_outdoor_cluster', 97, 92),
      _F('cable_junction_box', -130, 188), _F('cable_junction_box', 127, 188),
      _F('milk_crate_stack', 0, 188), _F('milk_crate_stack', 25, 88),
      _F('cat', -130, 60), _F('cat', 127, 60), _F('cat', 100, 165),
      // ── 境界・その他 ──
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 公共施設前のコンクリート帯 (公共性を地面で表現) ═══
      _G('concrete', 0, 60, 360, 28),

      // ═══ 各公共施設のロット (用途差を地面で) ═══
      // 保育園 (-110, 22): 砂地遊び場が大きい
      _G('tile', -110, 30, 60, 24),       // 玄関タイル
      _G('dirt', -110, 75, 100, 50),      // 砂地遊び場
      // 診療所 (0, 22): 整然とした
      _G('tile', 0, 30, 40, 24),
      // 郵便局 (110, 22): 駐車場あり
      ...
      // ═══ NE 上段周辺ロット (公共圏の周辺商店) ═══
      _G('grass', 60, 80, 30, 20),        // wagashi 庭
      _G('tile', 145, 78, 30, 28),        // townhouse 玄関
      _G('grass', -150, 80, 30, 20),      // greenhouse 庭

      // ═══ SW 下段住宅ロット ═══
      _G('grass', -170, 152, 40, 35),     // ロット house(-170, 138)
      _G('concrete', -100, 152, 50, 35),  // ロット kominka(-100, 130) 駐車
      _G('tile', -45, 152, 50, 35),       // ロット townhouse(-45, 138)

      // ═══ SE 下段住宅ロット ═══
      _G('grass', 60, 152, 50, 35),       // ロット house(60, 138)
      _G('tile', 100, 152, 50, 35),       // ロット duplex(100, 130) 玄関
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('parasol', -22, 154), _F('parasol', 19, 154), _F('parasol', 0, 168),
      _F('bench', 0, 160), _F('bench', -22, 162), _F('bench', 19, 162),
      _F('newspaper_stand', -22, 144), _F('newspaper_stand', 19, 144),
      _F('flower_planter_row', 0, 168), _F('potted_plant', 15, 152), _F('potted_plant', -18, 152),
      _F('bicycle_rack', 22, 176), _F('bicycle', 0, 176), _F('bicycle', -22, 176),
      _F('chouchin', 0, 122), _F('chouchin', 15, 122), _F('chouchin', -18, 122),
      // cafe
      _F('shop_awning', 109, 132), _F('a_frame_sign', 109, 144),
      _F('parasol', 92, 154), _F('parasol', 129, 154), _F('parasol', 112, 168),
      _F('bench', 92, 160), _F('bench', 129, 160), _F('bench', 109, 168),
      _F('flower_planter_row', 129, 168), _F('potted_plant', 109, 152), _F('potted_plant', 90, 152),
      _F('chouchin', 109, 122), _F('chouchin', 130, 122), _F('chouchin', 95, 122),
      _F('cat', 148, 172), _F('bicycle', 127, 176), _F('bicycle_rack', 92, 176),
      // ── テラス両端の店 (florist / 生活店) ──
      ...
      // ── 上段 裏路地 (生活痕跡) ──
      _F('garbage', -150, 88), _F('garbage', 147, 88), _F('garbage', -75, 92),
      ...
      _F('recycling_bin', -75, 188), _F('recycling_bin', 72, 188),
      _F('dumpster', -150, 188), _F('dumpster', 147, 188),
      _F('ac_outdoor_cluster', -55, 178), _F('ac_outdoor_cluster', 65, 178),
      _F('cable_junction_box', -130, 188), _F('cable_junction_box', 127, 188),
      _F('milk_crate_stack', -75, 178), _F('milk_crate_stack', 72, 178),
      _F('cat', -100, 178), _F('cat', 97, 178),
      // ── 連続軸: 桜並木 (Ch0-Ch5) ──
      ...
      // ── 境界・植栽 ──
      _F('hedge', -150, 195), _F('hedge', 147, 195), _F('hedge', 0, 195),
      _F('flower_bed', 0, 195), _F('flower_bed', -100, 192), _F('flower_bed', 97, 192),
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 3連テラスのウッドデッキ (商店街の主役地面) ═══
      _G('wood_deck', 0, 152, 310, 58),
      // 各店舗フロントのタイル (店ごとの領域感)
      _G('tile', -112, 138, 30, 12),
      _G('tile', 0, 138, 30, 12),
      _G('tile', 112, 138, 30, 12),

      // ═══ 上段商店ロット (用途差を地面で) ═══
      _G('tile', -160, 56, 36, 50),    // ロット shop(-160, 38)
      _G('tile', -110, 56, 36, 50),    // ロット townhouse(-110, 38)
      _G('grass', -75, 72, 36, 32),    // ロット cafe(-75, 60) 庭
      _G('tile', -45, 56, 38, 50),     // ロット florist(-45, 40)
      _G('tile', 42, 58, 38, 50),      // ロット laundromat(42, 40)
      _G('tile', 125, 58, 38, 50),     // ロット pharmacy(125, 40)

      // ═══ 商店裏の搬入路 ═══
      _G('asphalt', -130, 75, 30, 50),
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('bollard', -65, 92), _F('bollard', 62, 92),
      _F('guardrail_short', 32, 108), _F('guardrail_short', -35, 108),
      _F('street_mirror', -75, 92), _F('street_mirror', 72, 92),
      // ── タイトパッキング: 中間 dy 帯と裏路地 ──
      // dy=50 帯
      _F('hedge', -150, 50), _F('hedge', -75, 50), _F('hedge', 0, 50),
      _F('hedge', 72, 50), _F('hedge', 165, 50),
      _F('bush', -130, 60), _F('bush', -45, 60), _F('bush', 30, 60), _F('bush', 127, 60),
      _F('flower_bed', -100, 60), _F('flower_bed', 97, 60),
      _F('potted_plant', -75, 22), _F('potted_plant', 0, 22),
      // dy=110-118 中間帯
      _F('hedge', -150, 115), _F('hedge', 147, 115), _F('hedge', -45, 115), _F('hedge', 42, 115),
      _F('bush', -110, 118), _F('bush', 0, 118), _F('bush', 107, 118),
      // dy=160-175 中間帯
      _F('hedge', -130, 165), _F('hedge', -75, 165), _F('hedge', 72, 165),
      _F('hedge', 127, 165), _F('hedge', 0, 165),
      _F('bush', -100, 168), _F('bush', 97, 168),
      _F('flower_bed', -150, 175), _F('flower_bed', 147, 175),
      _F('flower_planter_row', -45, 175),
      // 裏路地家具
      _F('garbage', -150, 88), _F('garbage', 147, 88),
      _F('garbage', -45, 188), _F('garbage', 42, 188),
      _F('recycling_bin', -110, 88), _F('recycling_bin', 107, 88),
      _F('dumpster', 0, 88), _F('dumpster', -130, 188), _F('dumpster', 127, 188),
      _F('ac_outdoor_cluster', -75, 92), _F('ac_outdoor_cluster', 72, 92),
      _F('cable_junction_box', -130, 88), _F('cable_junction_box', 127, 88),
      _F('milk_crate_stack', -25, 88), _F('milk_crate_stack', 22, 88),
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 銭湯入口 (湯上がり客の溜まり) ═══
      _G('tile', -118, 42, 76, 44),
      _G('tile', -118, 78, 60, 14),

      // ═══ 各商店ロット (用途差を地面で) ═══
      _G('tile', -75, 58, 40, 34),     // shop ロット
      _G('asphalt', 45, 58, 90, 34),   // supermarket 駐車場
      _G('tile', 120, 58, 40, 34),     // pharmacy ロット

      // ═══ ラーメン店テラス (wood_deck で店先感) ═══
      _G('wood_deck', -40, 138, 50, 32),

      // ═══ SW 下段住宅ロット ═══
      _G('grass', -160, 152, 40, 35),    // ロット house(-160, 132)
      _G('tile', -110, 152, 40, 35),     // ロット townhouse(-110, 138)

      // ═══ SE 下段住宅ロット ═══
      _G('concrete', 95, 152, 50, 35),   // ロット townhouse(95, 132) 駐車
      ...
      // ═══ 落ち葉アクセント ═══
      _G('fallen_leaves', -110, 175, 18, 12),
    ],
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('bench', 0, 178), _F('bench', -110, 178), _F('bench', 107, 178),
      _F('bench', -50, 168), _F('bench', 47, 168),
      _F('flower_bed', -110, 140), _F('flower_bed', 107, 140), _F('flower_bed', 0, 195),
      ...
      // library 帰属
      _F('shop_awning', 140, 30), _F('flag_pole', 147, 62),
      ...
      _F('power_pole', 178, 195), _F('power_line', 175, 192),
      // ── avenue 地点小物 + 街灯・ガードレール ──
      _F('manhole_cover', 0, 100), _F('manhole_cover', 57, 100), _F('manhole_cover', -60, 100),
      _F('bollard', -65, 92), _F('bollard', 62, 92),
      ...
      _F('bush', -130, 22), _F('bush', 0, 22), _F('bush', 127, 22),
      _F('potted_plant', -100, 30), _F('potted_plant', 30, 30),
      // dy=60-72 帯 (公共施設前のタイル境界)
      _F('hedge', -130, 70), _F('hedge', -75, 70), _F('hedge', 0, 70),
      _F('hedge', 72, 70), _F('hedge', 127, 70),
      _F('bush', -160, 70), _F('bush', 165, 70), _F('bush', -45, 70), _F('bush', 42, 70),
      _F('flower_bed', -100, 70), _F('flower_bed', 97, 70),
      _F('flower_planter_row', -150, 65), _F('flower_planter_row', 147, 65),
      // dy=110-118 中間帯
      _F('hedge', -150, 115), _F('hedge', 147, 115), _F('hedge', -45, 115), _F('hedge', 42, 115),
      _F('bush', -100, 118), _F('bush', 97, 118), _F('bush', 0, 118),
      // dy=160-168 中間帯 (広場外周)
      _F('hedge', -130, 165), _F('hedge', 127, 165),
      _F('hedge', -75, 175), _F('hedge', 72, 175),
      _F('bush', -150, 168), _F('bush', 147, 168),
      _F('flower_bed', -45, 175), _F('flower_bed', 42, 175),
      _F('flower_planter_row', -100, 175), _F('flower_planter_row', 97, 175),
      // 裏路地家具
      _F('garbage', -150, 88), _F('garbage', 147, 88),
      _F('garbage', -75, 188), _F('garbage', 72, 188),
      _F('recycling_bin', -130, 88), _F('recycling_bin', 127, 88),
      _F('dumpster', 0, 88), _F('dumpster', -45, 188), _F('dumpster', 42, 188),
      _F('ac_outdoor_cluster', -100, 92), _F('ac_outdoor_cluster', 97, 92),
      _F('cable_junction_box', -130, 188), _F('cable_junction_box', 127, 188),
      _F('milk_crate_stack', -25, 88), _F('milk_crate_stack', 22, 88),
      _F('cat', -130, 60), _F('cat', 127, 60), _F('cat', 100, 178), _F('cat', -110, 168),
      // 広場外周 enrichment
      _F('bench', -150, 168), _F('bench', 130, 168),
      _F('flower_bed', -50, 130), _F('flower_bed', 47, 130),
      _F('potted_plant', -50, 145), _F('potted_plant', 47, 145),
      // ── 境界 ──
      _F('hedge', -160, 195), _F('hedge', 165, 195),
      _F('hedge', -130, 110), _F('hedge', 127, 110),
    ],
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 中央広場の石畳 (大きな面で広場の中心) ═══
      _G('stone_pavement', 0, 150, 280, 100),
      _G('tile', 0, 145, 130, 60),  // 中央タイル円
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('guardrail_short', -35, 100), _F('guardrail_short', 32, 100),
      _F('bollard', -55, 92), _F('bollard', 52, 92),
      ...
      // ── 連続軸: tree (Ch6-Ch8) ──
      _F('tree', -170, 110), _F('tree', 25, 110), _F('tree', 167, 110),
      ...
      _F('hedge', -45, 115), _F('hedge', 42, 115), _F('hedge', 145, 115),
      _F('bush', -100, 118), _F('bush', 0, 118), _F('bush', 97, 118),
      _F('manhole_cover', -100, 108), _F('manhole_cover', 97, 108),
      // dy=160-175 中間帯
      _F('hedge', -130, 165), _F('hedge', 0, 165), _F('hedge', 127, 165),
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 大きな校庭の土 (左下半分以上) ═══
      _G('dirt', -85, 150, 200, 90),
      _G('dirt', -135, 175, 30, 22),  // 砂場
      _G('dirt', -50, 175, 26, 18),   // 砂場2

      // ═══ 学校・保育園の入口タイル ═══
      _G('tile', -70, 70, 120, 36),  // 校門
      _G('tile', 95, 60, 60, 38),    // 保育園入口
      ...
      // ═══ 東側の商店ロット (用途差) ═══
      _G('tile', 35, 130, 50, 24),    // cafe ロット
      _G('tile', 80, 130, 36, 24),    // bookstore ロット
      _G('tile', 145, 130, 36, 24),   // shop ロット
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('chouchin', 25, 34), _F('chouchin', 62, 34), _F('chouchin', 85, 34),
      _F('chouchin', 107, 34), _F('chouchin', 129, 34), _F('chouchin', 142, 34),
      _F('chouchin', 165, 34),
      _F('chouchin', -110, 90), _F('chouchin', -40, 90), _F('chouchin', 0, 90),
      _F('chouchin', 33, 90), _F('chouchin', 115, 90), _F('chouchin', 145, 90),
      _F('chouchin', -150, 90), _F('chouchin', -68, 90), _F('chouchin', 78, 90),
      _F('banner_pole', -55, 95), _F('banner_pole', 52, 95), _F('banner_pole', 0, 60),
      _F('banner_pole', -150, 60), _F('banner_pole', 147, 60),
      // ── 各店の帰属 (オーニング・暖簾・看板) ──
      // ramen
      ...
      _F('bench', 35, 62), _F('flower_planter_row', 32, 60),
      _F('bench', 22, 70), _F('bench', 50, 70), _F('potted_plant', 22, 56),
      _F('chouchin', 35, 22),
      // shop
      _F('shop_awning', 92, 30), _F('a_frame_sign', 92, 56),
      _F('vending', 117, 56), _F('bicycle_rack', 70, 56),
      _F('bicycle', 70, 70), _F('bicycle', 95, 70), _F('potted_plant', 75, 56),
      _F('flower_planter_row', 95, 60), _F('chouchin', 92, 22),
      // game_center
      _F('shop_awning', 150, 30), _F('a_frame_sign', 150, 56),
      _F('chouchin', 150, 22), _F('vending', 165, 56),
      _F('bicycle_row', 130, 60), _F('bicycle', 127, 70), _F('bicycle', 165, 70),
      _F('potted_plant', 127, 56), _F('newspaper_stand', 165, 70),
      // ── 上段 裏側 (店舗の奥) ──
      _F('garbage', -160, 92), _F('garbage', 157, 92), _F('garbage', -75, 92),
      _F('garbage', 72, 92),
      _F('recycling_bin', -110, 92), _F('recycling_bin', 25, 92),
      _F('dumpster', -130, 92), _F('dumpster', 127, 92),
      _F('ac_outdoor_cluster', -75, 88), _F('ac_outdoor_cluster', 72, 88),
      _F('ac_outdoor_cluster', -25, 88), _F('ac_outdoor_cluster', 110, 88),
      _F('cable_junction_box', -130, 88), _F('cable_junction_box', 127, 88),
      _F('cable_junction_box', -55, 88), _F('cable_junction_box', 52, 88),
      _F('milk_crate_stack', -110, 88), _F('milk_crate_stack', 107, 88),
      _F('milk_crate_stack', -25, 92), _F('milk_crate_stack', 22, 92),
      _F('cat', -80, 88), _F('cat', 77, 88),
      // ── 下段 住宅 facade ──
      _F('mailbox', -140, 120), _F('mailbox', -100, 122), _F('mailbox', -45, 122),
      _F('mailbox', 40, 122), _F('mailbox', 110, 122), _F('mailbox', 165, 120),
      _F('mailbox', -75, 152), _F('mailbox', 72, 152),
      _F('ac_unit', -140, 148), _F('ac_unit', 110, 148), _F('ac_unit', -45, 158),
      _F('ac_unit', 165, 158), _F('ac_unit', -100, 148), _F('ac_unit', 40, 158),
      _F('ac_unit', -75, 178), _F('ac_unit', 72, 178),
      ...
      _F('garbage', -75, 188), _F('garbage', 72, 188),
      _F('recycling_bin', -110, 188), _F('recycling_bin', 107, 188),
      _F('cable_junction_box', 127, 168), _F('ac_outdoor_cluster', -45, 168),
      _F('ac_outdoor_cluster', 40, 168), _F('cable_junction_box', -130, 168),
      _F('milk_crate_stack', -75, 188), _F('milk_crate_stack', 72, 188),
      _F('milk_crate_stack', -22, 188), _F('milk_crate_stack', 19, 188),
      _F('bicycle', -100, 168), _F('bicycle', 97, 168),
      _F('bicycle_rack', -22, 168), _F('bicycle_rack', 19, 168),
      _F('cat', -80, 170), _F('cat', 77, 170),
      // ── 連続軸: tree (Ch6-Ch8) ──
      _F('tree', -170, 110), _F('tree', 167, 110), _F('tree', 0, 110),
      _F('tree', -75, 188), _F('tree', 72, 188), _F('tree', 127, 188),
      ...
      _F('flower_bed', -130, 195), _F('flower_bed', 110, 195),
      _F('flower_bed', -45, 195), _F('flower_bed', 50, 195),
    ],
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: アーケードの長い wood_deck (6店舗分の歩行帯) ═══
      _G('wood_deck', 0, 58, 340, 58),
      _G('tile', 0, 92, 340, 28),  // アーケード下タイル帯

      // ═══ 各店舗ロット (アーケード上の用途差) ═══
      _G('tile', -145, 56, 32, 14),    // ramen
      _G('tile', -95, 56, 32, 14),     // izakaya
      _G('tile', -35, 56, 32, 14),     // bookstore
      _G('tile', 35, 56, 32, 14),      // cafe
      _G('tile', 95, 56, 32, 14),      // shop
      _G('tile', 145, 56, 32, 14),     // game_center

      // ═══ SW 下段住宅ロット ═══
      _G('grass', -140, 152, 36, 32),    // ロット house(-140, 132)
      _G('tile', -100, 152, 30, 32),     // ロット townhouse(-100, 138)
      _G('concrete', -45, 152, 36, 32),  // ロット house(-45, 138)

      // ═══ SE 下段住宅ロット ═══
      _G('concrete', 40, 152, 36, 32),   // ロット townhouse(40, 138)
      _G('tile', 110, 152, 36, 32),      // ロット house(110, 138)
      ...
      // ═══ 落ち葉アクセント ═══
      _G('fallen_leaves', -160, 188, 26, 12),
      _G('fallen_leaves', 160, 188, 26, 12),
    ],
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      // dy=110-118 中間帯
      _F('hedge', -150, 115), _F('hedge', -45, 115), _F('hedge', 42, 115),
      _F('bush', -100, 118), _F('bush', 0, 118), _F('bush', 97, 118),
      // dy=160-175 中間帯
      _F('hedge', -130, 165), _F('hedge', 0, 165), _F('hedge', 65, 175),
      _F('hedge', 127, 165), _F('hedge', 165, 165),
      _F('bush', -100, 168), _F('bush', 97, 168),
      ...
      _F('cat', -130, 60), _F('cat', 127, 60), _F('cat', 60, 168),
      _F('cat', -55, 188),
      // ── 境界 ──
      _F('flower_bed', 165, 188), _F('hedge', -170, 110), _F('hedge', 167, 110),
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 駅前ロータリーのアスファルト (右側) ═══
      _G('asphalt', 95, 70, 170, 80),
      _G('stone_pavement', 95, 52, 130, 42),  // 駅舎ファサード前
      _G('stone_pavement', 130, 88, 80, 16),   // ロータリー外周

      // ═══ 駅前商店ロット (3 店舗の用途差) ═══
      _G('tile', -160, 56, 40, 38),    // shop ロット
      _G('tile', -120, 56, 50, 38),    // bookstore ロット
      _G('tile', -45, 56, 45, 38),     // cafe ロット

      // ═══ ラーメン店テラス ═══
      _G('wood_deck', 50, 138, 50, 26),

      // ═══ SW 下段住宅ロット ═══
      _G('grass', -160, 152, 40, 32),    // ロット house(-160, 138)
      _G('concrete', -120, 152, 50, 32), // ロット convenience(-120, 138) 駐車
      _G('tile', -40, 152, 36, 32),      // ロット shop(-40, 138)

      // ═══ SE 下段住宅ロット ═══
      _G('grass', 130, 152, 40, 32),     // ロット townhouse(130, 132)
      _G('tile', 165, 152, 30, 32),      // ロット house(165, 138)
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('wood_fence', -25, 70), _F('wood_fence', 22, 70),
      // dy=110-118 中間帯
      _F('hedge', -150, 115), _F('hedge', -45, 115), _F('hedge', 42, 115),
      _F('bush', -100, 118), _F('bush', 0, 118), _F('bush', 97, 118),
      // dy=160-175 (畑の縁)
      _F('hedge', -100, 165), _F('hedge', 0, 165), _F('hedge', 130, 165),
      _F('bush', -150, 168), _F('bush', 60, 168), _F('bush', 147, 168),
      ...
      _F('dumpster', 130, 88),
      _F('ac_outdoor_cluster', -75, 92), _F('ac_outdoor_cluster', 72, 92),
      _F('cable_junction_box', -45, 88), _F('cable_junction_box', 42, 88),
      _F('milk_crate_stack', 22, 88), _F('milk_crate_stack', -25, 88),
      _F('pallet_stack', 60, 88), _F('pallet_stack', -150, 168),
      _F('drum_can', 0, 88), _F('drum_can', -130, 88),
      _F('cat', -130, 60), _F('cat', 127, 60), _F('cat', -150, 168),
      // ── 境界 ──
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('cable_junction_box', 165, 76), _F('cable_junction_box', 97, 84),
      _F('drum_can', 25, 60), _F('bollard', 50, 60),
      // ── 道路境界 ──
      _F('traffic_light', 0, 92),
      _F('guardrail_short', -160, 100), _F('guardrail_short', 157, 100),
      ...
      _F('garbage', -150, 88), _F('garbage', 165, 88),
      _F('garbage', -45, 188), _F('garbage', 42, 188),
      _F('recycling_bin', -100, 88), _F('recycling_bin', 97, 88),
      _F('dumpster', -130, 188), _F('dumpster', 127, 188),
      _F('ac_outdoor_cluster', -75, 92), _F('ac_outdoor_cluster', 72, 92),
      _F('cable_junction_box', -130, 88), _F('cable_junction_box', 127, 88),
      _F('milk_crate_stack', -25, 88), _F('milk_crate_stack', 22, 88),
    ...
    grounds: [
      ...
      // ═══ 焦点ゾーン: 倉庫前のコンクリヤード (敷地別) ═══
      _G('concrete', -90, 62, 170, 70),    // 倉庫2連の前ヤード
      _G('concrete', 95, 62, 160, 70),     // 消防・警察詰所の前

      // ═══ SW 下段ロット ═══
      _G('concrete', -135, 152, 80, 35),   // ガレージ・住宅前
      _G('grass', -170, 175, 30, 18),      // 西端の緑

      // ═══ SE 下段住宅ロット ═══
      _G('grass', 95, 152, 50, 32),        // 住宅 house(95, 132) 庭
      _G('tile', 145, 152, 50, 32),        // 住宅 townhouse(145, 138) 玄関

      // ═══ 倉庫敷地内の搬入アスファルト ═══
      _G('asphalt', 50, 138, 60, 30),
      _G('asphalt', -45, 138, 30, 30),
  ...
  { patternId: 's1_raw', raw: {
    ...
    furniture: [
      ...
      _F('bush', -100, 118), _F('bush', 0, 118), _F('bush', 97, 118),
      // dy=160-175 中間帯 (踏切手前)
      _F('hedge', -130, 165), _F('hedge', -75, 165), _F('hedge', 72, 165), _F('hedge', 127, 165),
      _F('bush', -100, 168), _F('bush', 0, 168), _F('bush', 97, 168),
      _F('flower_bed', -160, 175),
      // 裏路地家具
      _F('garbage', -130, 88), _F('garbage', 127, 88),
      ...
      _F('guardrail_short', -55, 198), _F('guardrail_short', 52, 198),
      _F('guardrail_short', -135, 198), _F('guardrail_short', 132, 198),
      _F('manhole_cover', -30, 198), _F('manhole_cover', 27, 198),
      _F('bollard', -65, 195), _F('bollard', 62, 195),
    ...
    grounds: [
      ...
      _G('asphalt', 0, 152, 300, 14),    // 線路の路盤 1
      _G('asphalt', 0, 165, 300, 14),    // 線路の路盤 2

      // ═══ 各店舗ロット (用途差) ═══
      _G('concrete', -135, 60, 70, 38),  // ガソリンスタンド
      _G('concrete', -45, 58, 80, 40),   // コンビニ前
      _G('tile', 70, 58, 50, 38),        // ラーメン店前
      _G('tile', 130, 70, 40, 32),       // 着物屋の店先

      // ═══ 踏切手前の古い町並みロット ═══
      _G('residential_tile', -120, 138, 60, 50),  // machiya 下
      _G('residential_tile', -50, 138, 50, 30),   // townhouse(-50, 132) ロット
      _G('residential_tile', 145, 138, 50, 30),   // house(145, 132) ロット
...
// ═══════════════════════════════════════════════════════════════════
// Stage 2: 夜の歓楽街 NEON DISTRICT (12 chunks, raw 配置, v1.0 設計指示書準拠)
// 連続軸: chouchin 帯 / noren / puddle_reflection / 街灯ネオン
// ground 基調: asphalt + Ch10-11 で stone_pavement 移行
// 道路ポリシー: 中央 _AVE のみ固定、横道路はチャンクごと (v6.2 継承)
// ═══════════════════════════════════════════════════════════════════
const STAGE_2_TEMPLATES: ChunkTemplate[] = [

  // ── S2-Ch0: 終電後の地方駅と北口ロータリー (NE hero) ──
  // Stage 1 末尾の踏切 → 夜の駅前広場へ切替
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // NE hero: train_station + 駅前広場
    const station = $B(out, 'train_station', 80, 22);
    $B(out, 'business_hotel', -130, 22);   // NW
    $B(out, 'ramen', -100, 130);           // SW
    $B(out, 'convenience', 130, 130);      // SE
    $B(out, 'apartment', -160, 60);
    $B(out, 'office', -45, 22);

    // NE 駅前焦点取り巻き
    const platform = $F(out, 'platform_edge', 80, 70);
    const track1 = $F(out, 'railway_track', 80, 78);
    const track2 = $F(out, 'railway_track', 80, 86);
    const signal = $F(out, 'signal_tower', 80, 95);
    const flagPole = $F(out, 'flag_pole', 30, 62);
    const benchN1 = $F(out, 'bench', 20, 85);
    const benchN2 = $F(out, 'bench', 65, 85);
    const benchN3 = $F(out, 'bench', 120, 85);
    const planter1 = $F(out, 'flower_planter_row', 80, 60);
    const planter2 = $F(out, 'flower_planter_row', 80, 92);
    const taxiSign = $F(out, 'taxi_rank_sign', 130, 72);
    const busStop1 = $F(out, 'bus_stop', 30, 68);
    const busStop2 = $F(out, 'bus_stop', 130, 68);
    const newsStand = $F(out, 'newspaper_stand', 110, 88);  // NE livingTrace

    // NW business_hotel facade
    $F(out, 'sign_board', -130, 8);
    const nwMail = $F(out, 'mailbox', -150, 22);  // NW livingTrace
    $F(out, 'flag_pole', -100, 62);

    // SW ramen
    $F(out, 'chouchin', -120, 118);
    $F(out, 'chouchin', -80, 118);
    $F(out, 'noren', -100, 122);
    ...
    const seGarbage = $F(out, 'garbage', 160, 188);  // SE livingTrace

    // 連続軸: chouchin 帯 (上空 y=12)
    $F(out, 'chouchin', -160, 15); $F(out, 'chouchin', -90, 15);
    $F(out, 'chouchin', 87, 15); $F(out, 'chouchin', 155, 15);

    // 街灯ネオン + puddle (連続軸)
    $F(out, 'street_lamp', -90, 88); $F(out, 'street_lamp', 90, 88);
    $F(out, 'street_lamp', -90, 108); $F(out, 'street_lamp', 90, 108);
    $F(out, 'puddle_reflection', -45, 95);
    $F(out, 'puddle_reflection', 45, 105);
    $F(out, 'puddle_reflection', -100, 145);

    // bollard 横断帯 (鏡像対称避け、§6 準拠)
    $F(out, 'bollard', -65, 92); $F(out, 'bollard', 62, 92);
    $F(out, 'manhole_cover', -30, 100); $F(out, 'manhole_cover', 33, 100);
    ...
    out.humans = [
      _H(80, 65), _H(80, 80),                    // 駅構内
      _H(35, 85), _H(120, 85), _H(70, 80),       // 駅前
      _H(-30, 100), _H(40, 100), _H(-90, 100),   // avenue 通勤客
      _H(-100, 145), _H(-75, 152),               // ramen
      _H(130, 152), _H(115, 158),                // convenience
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),                  // 基調
      _G('concrete', 80, 80, 180, 120),                 // NE 駅舎前
      _G('tile', 80, 70, 80, 20),                       // ホーム
      _G('stone_pavement', -65, 100, 12, 200),          // 中央生活路地 (Stage 1 継続)
      _G('asphalt', -100, 152, 60, 30),                 // SW ramen 前
      _G('tile', 130, 152, 50, 30),                     // SE convenience 前
    ];

    _CLUSTER(out, {
      id: 's2.ch0.NE.station', role: 'hero', cell: 'NE',
      focal: station,
      companions: [platform, track1, track2, signal, flagPole],
      boundary: [planter1, planter2, benchN1, benchN3],
      access: [taxiSign, busStop1, busStop2],
      livingTrace: newsStand,
    });
    _CLUSTER(out, {
      id: 's2.ch0.NW.hotel', role: 'ambient', cell: 'NW',
      focal: { kind: 'b', i: 1 },  // business_hotel
      companions: [{ kind: 'f', i: 16 }],  // sign_board NW
      livingTrace: nwMail,
    });
    _CLUSTER(out, {
      id: 's2.ch0.SW.ramen', role: 'ambient', cell: 'SW',
      focal: { kind: 'b', i: 2 },  // ramen
      companions: [
        { kind: 'f', i: 19 },  // chouchin SW1
        { kind: 'f', i: 20 },  // chouchin SW2
        { kind: 'f', i: 21 },  // noren
      ],
      livingTrace: swBike,
    });
    _CLUSTER(out, {
      id: 's2.ch0.SE.convenience', role: 'ambient', cell: 'SE',
      focal: { kind: 'b', i: 3 },  // convenience
      companions: [
        { kind: 'f', i: 24 },  // shop_awning
        { kind: 'f', i: 25 },  // vending
        { kind: 'f', i: 26 },  // bicycle_rack
      ],
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // ambient
    $B(out, 'business_hotel', -130, 22);  // NW
    $B(out, 'convenience', 110, 22);      // NE
    $B(out, 'townhouse', 80, 130);        // SE
    $B(out, 'townhouse', 140, 130);
    $B(out, 'apartment', -50, 22);
    $B(out, 'shop', 165, 22);

    // SW 焦点取り巻き (各店 chouchin + noren + a_frame_sign)
    const ramenChouchinL = $F(out, 'chouchin', -160, 118);
    const ramenChouchinR = $F(out, 'chouchin', -130, 118);
    $F(out, 'noren', -145, 125);
    $F(out, 'a_frame_sign', -145, 148);
    const izakayaChouchinL = $F(out, 'chouchin', -110, 118);
    const izakayaChouchinR = $F(out, 'chouchin', -80, 118);
    $F(out, 'noren', -95, 125);
    $F(out, 'a_frame_sign', -95, 148);
    $F(out, 'sign_board', -35, 110);
    $F(out, 'a_frame_sign', -35, 148);
    $F(out, 'chouchin', -50, 118); $F(out, 'chouchin', -20, 118);
    // 焦点囲み
    const sw_b1 = $F(out, 'bollard', -160, 110);
    const sw_b2 = $F(out, 'bollard', -10, 110);
    const sw_p1 = $F(out, 'flower_planter_row', -120, 110);
    const sw_p2 = $F(out, 'flower_planter_row', -65, 110);
    // access (動線)
    const sw_lamp1 = $F(out, 'street_lamp', -90, 130);
    const sw_lamp2 = $F(out, 'street_lamp', -10, 130);
    const sw_pud1 = $F(out, 'puddle_reflection', -130, 145);
    const sw_pud2 = $F(out, 'puddle_reflection', -60, 148);
    const sw_pud3 = $F(out, 'puddle_reflection', -20, 145);
    const sw_garbage = $F(out, 'garbage', -158, 162);  // SW livingTrace
    ...
    // 連続軸: 上空 chouchin 帯 (avenue 沿い)
    $F(out, 'chouchin', -160, 15); $F(out, 'chouchin', -90, 15);
    $F(out, 'chouchin', 87, 15); $F(out, 'chouchin', 157, 15);
    // 街灯
    $F(out, 'street_lamp', -90, 88); $F(out, 'street_lamp', 90, 88);
    $F(out, 'street_lamp', -90, 108); $F(out, 'street_lamp', 90, 108);
    // bollard 横断帯
    ...
    out.humans = [
      _H(-145, 145), _H(-95, 145), _H(-35, 145),  // 各店客
      _H(-130, 158), _H(-65, 158), _H(-15, 158),
      _H(-30, 100), _H(40, 100), _H(-90, 100),    // avenue 通勤
      _H(110, 56),                                  // NE convenience
      _H(80, 152),                                  // SE townhouse
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('concrete', -90, 145, 160, 40),       // SW 3 連店前
      _G('stone_pavement', -65, 100, 12, 200),
      _G('tile', 110, 56, 50, 40),              // NE convenience
    ];

    _CLUSTER(out, {
      id: 's2.ch1.SW.dining_trio', role: 'hero', cell: 'SW',
      focal: izakaya,  // 中央
      companions: [ramen, karaoke, ramenChouchinL, izakayaChouchinR],
      boundary: [sw_b1, sw_b2, sw_p1, sw_p2],
      access: [sw_lamp1, sw_lamp2, sw_pud1, sw_pud2],
      livingTrace: sw_garbage,
    });
    _CLUSTER(out, {
      id: 's2.ch1.NW.hotel', role: 'ambient', cell: 'NW',
      focal: { kind: 'b', i: 3 },  // business_hotel
      companions: [{ kind: 'f', i: 26 }],  // sign_board NW
      livingTrace: nwMail,
    });
    _CLUSTER(out, {
      id: 's2.ch1.NE.convenience', role: 'ambient', cell: 'NE',
      focal: { kind: 'b', i: 4 },  // convenience
      companions: [{ kind: 'f', i: 28 }, { kind: 'f', i: 29 }],
      livingTrace: neBike,
    });
    _CLUSTER(out, {
      id: 's2.ch1.SE.townhouse', role: 'ambient', cell: 'SE',
      focal: { kind: 'b', i: 5 },  // townhouse
      companions: [{ kind: 'b', i: 6 }, { kind: 'f', i: 33 }],
      livingTrace: seCat,
    });
  ...
  // ── S2-Ch2: 商店街アーケード入口 (NW+NE merged hero) ──
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    const out: RawChunkBody = {
      buildings: [], furniture: [], humans: [], grounds: [],
      horizontalRoads: [_MID_HR, _TOP_HR],  // Act 境界予告
      verticalRoads: [_AVE],
    };
    // merged hero: shotengai_arcade × 2 (avenue 両端でゲート)
    const arcadeW = $B(out, 'shotengai_arcade', -118, 22);
    const arcadeE = $B(out, 'shotengai_arcade', 118, 22);
    // ambient (下段)
    const pachinkoPrev = $B(out, 'pachinko', -130, 130);  // SW Ch4 予告
    const karaokePrev = $B(out, 'karaoke', 130, 130);     // SE Ch3-4 予告
    $B(out, 'office', -50, 22);
    $B(out, 'office', 50, 22);
    $B(out, 'townhouse', -100, 178);
    $B(out, 'townhouse', 100, 178);

    // merged 焦点: chouchin 帯 14 本 (avenue 全幅、不揃い間隔で行進回避)
    ...
    const bp2 = $F(out, 'banner_pole', -50, 28);
    const bp3 = $F(out, 'banner_pole', 47, 28);
    const bp4 = $F(out, 'banner_pole', 157, 28);
    // 境界 flag_pole + flower_planter_row
    ...
    $F(out, 'puddle_reflection', -100, 145);
    // SW/SE 痕跡 (Ch4 予告の sign_board)
    const swSign = $F(out, 'sign_board', -130, 110);  // pachinko ネオン
    const seSign = $F(out, 'sign_board', 127, 110);   // karaoke ネオン
    const swGarbage = $F(out, 'garbage', -158, 188);
    const seBike = $F(out, 'bicycle', 158, 188);
    // bollard
    $F(out, 'bollard', -65, 92); $F(out, 'bollard', 62, 92);
    $F(out, 'manhole_cover', -30, 100); $F(out, 'manhole_cover', 33, 100);
    // 電柱
    ...
    out.humans = [
      _H(-90, 100), _H(0, 100), _H(90, 100),  // avenue 多
      _H(-45, 92), _H(48, 108), _H(-100, 88),
      _H(120, 92), _H(-130, 152), _H(130, 152),
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('tile', 0, 100, 360, 24),              // アーケード床
      _G('stone_pavement', -65, 100, 12, 200),
    ...
    _CLUSTER(out, {
      id: 's2.ch2.merged.arcade_gate', role: 'hero', cell: 'merged',
      focal: c1,  // 提灯帯起点
      companions: [c14, arcadeW, arcadeE, bp1],
      boundary: [bp2, bp3, bp4, flag1, flag2, fp1, fp2],
      access: [lamp1, lamp2, pud1, pud2],
      livingTrace: swGarbage,
    });
    _CLUSTER(out, {
      id: 's2.ch2.SW.pachinko_preview', role: 'ambient', cell: 'SW',
      focal: pachinkoPrev,
      companions: [swSign],
      livingTrace: swGarbage,
    });
    _CLUSTER(out, {
      id: 's2.ch2.SE.karaoke_preview', role: 'ambient', cell: 'SE',
      focal: karaokePrev,
      companions: [seSign],
      livingTrace: seBike,
    });
  ...
  // ── S2-Ch3: 高層雑居ビル (NE hero: karaoke + apartment_tall) ──
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    const out: RawChunkBody = {
      buildings: [], furniture: [], humans: [], grounds: [],
      horizontalRoads: [_MID_HR],
      verticalRoads: [_AVE, _VR(-90, 0, 100)],  // 雑居ビル裏路地
    };
    // NE hero: 高層
    const karaoke = $B(out, 'karaoke', 130, 22);
    const aptTall = $B(out, 'apartment_tall', 80, 22);
    // ambient
    const club = $B(out, 'club', -130, 22);     // NW
    $B(out, 'business_hotel', -100, 130);       // SW
    $B(out, 'mansion', 80, 130);                 // SE
    $B(out, 'mansion', 140, 130);
    $B(out, 'office', -50, 22);
    $B(out, 'apartment', 30, 22);

    // NE 焦点取り巻き
    const karaokeBigSign = $F(out, 'sign_board', 127, 8);  // 大ネオン
    const c1 = $F(out, 'chouchin', 80, 58); $F(out, 'chouchin', 105, 58);
    $F(out, 'chouchin', 130, 58); $F(out, 'chouchin', 155, 58);
    ...
    const b3 = $F(out, 'bollard', 165, 92);
    // 動線
    const lampNE1 = $F(out, 'street_lamp', 80, 88);
    const lampNE2 = $F(out, 'street_lamp', 165, 88);
    const pud1 = $F(out, 'puddle_reflection', 95, 115);
    const pud2 = $F(out, 'puddle_reflection', 145, 110);
    const recyc = $F(out, 'recycling_bin', 165, 158);  // NE livingTrace

    // NW club
    $F(out, 'sign_board', -130, 8);  // 黒+金
    ...
    const seBike = $F(out, 'bicycle_rack', 105, 168);

    // 連続軸: chouchin 上空 + 街灯ネオン
    $F(out, 'chouchin', -160, 15); $F(out, 'chouchin', 0, 15);
    $F(out, 'chouchin', 165, 15);
    ...
    out.humans = [
      _H(130, 60), _H(110, 80), _H(80, 60), _H(165, 80),  // NE karaoke 客
      _H(-130, 60), _H(-150, 80), _H(-100, 80),            // NW club
      _H(-30, 100), _H(40, 100),                            // avenue
      _H(-100, 152), _H(80, 152), _H(140, 152),
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('red_carpet', -130, 60, 60, 20),    // NW club 入口
      _G('concrete', 130, 60, 60, 30),        // NE karaoke 前
      _G('stone_pavement', -65, 100, 12, 200),
      _G('tile', 105, 152, 100, 30),
    ];

    _CLUSTER(out, {
      id: 's2.ch3.NE.karaoke_tall', role: 'hero', cell: 'NE',
      focal: karaoke,
      companions: [aptTall, karaokeBigSign, c1, flag],
      boundary: [b1, b2, b3, a1, a2],
      access: [lampNE1, lampNE2, pud1, pud2],
      livingTrace: recyc,
    });
    _CLUSTER(out, {
      id: 's2.ch3.NW.club', role: 'ambient', cell: 'NW',
      focal: club,
      companions: [{ kind: 'f', i: 14 }, { kind: 'f', i: 15 }, { kind: 'f', i: 16 }],
      livingTrace: nwRecyc,
    });
    _CLUSTER(out, {
      id: 's2.ch3.SW.hotel', role: 'ambient', cell: 'SW',
      focal: { kind: 'b', i: 3 },  // business_hotel
      companions: [{ kind: 'f', i: 19 }],
      livingTrace: swMail,
    });
    _CLUSTER(out, {
      id: 's2.ch3.SE.mansions', role: 'ambient', cell: 'SE',
      focal: { kind: 'b', i: 4 },  // mansion
      companions: [{ kind: 'b', i: 5 }],
      livingTrace: seBike,
    });
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // merged hero: パチンコ + 交番 + ゲーセン
    const pachinko = $B(out, 'pachinko', -100, 22);
    const koban = $B(out, 'police_station', 0, 130);
    const gameCenter = $B(out, 'game_center', 100, 22);
    // 周辺の店
    $B(out, 'love_hotel', -80, 130);
    $B(out, 'business_hotel', 110, 130);
    $B(out, 'snack', -160, 22);
    $B(out, 'izakaya', 160, 22);

    // 焦点取り巻き
    const pachinkoBigSign = $F(out, 'sign_board', -100, 8);  // 巨大ネオン
    const flagKoban1 = $F(out, 'flag_pole', -12, 120);
    const flagKoban2 = $F(out, 'flag_pole', 12, 120);
    const cone1 = $F(out, 'traffic_cone', -25, 145);
    ...
    // 動線
    const lamp1 = $F(out, 'street_lamp', -90, 88); $F(out, 'street_lamp', 90, 88);
    const lamp2 = $F(out, 'street_lamp', -90, 108); $F(out, 'street_lamp', 90, 108);
    const mirror1 = $F(out, 'street_mirror', -45, 92);
    const mirror2 = $F(out, 'street_mirror', 48, 92);
    const pud1 = $F(out, 'puddle_reflection', -100, 60);
    const pud2 = $F(out, 'puddle_reflection', 97, 60);
    ...
    $F(out, 'sign_board', 157, 8);     // izakaya
    $F(out, 'chouchin', 157, 30); $F(out, 'noren', 160, 32);
    $F(out, 'sign_board', -80, 110);   // love_hotel
    $F(out, 'parasol', -80, 148);
    ...
    out.humans = [
      _H(-100, 60), _H(-80, 80), _H(-130, 80),  // パチンコ客
      _H(-115, 95), _H(-105, 95),                // 行列
      _H(0, 145), _H(-15, 130),                   // 交番
      _H(100, 60), _H(85, 80),                    // ゲーセン
      _H(-30, 100), _H(40, 100), _H(-90, 100), _H(90, 100),  // avenue 多
      _H(-80, 152), _H(110, 152), _H(-160, 60),
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('tile', -100, 60, 90, 50),         // パチンコ前
      _G('concrete', 0, 130, 80, 50),        // 交番前
      _G('tile', 100, 60, 90, 50),           // ゲーセン
      _G('stone_pavement', -65, 100, 12, 200),
      _G('red_carpet', -80, 152, 50, 30),    // love_hotel
    ];

    _CLUSTER(out, {
      id: 's2.ch4.merged.pachinko_koban', role: 'hero', cell: 'merged',
      focal: pachinko,  // クライマックス
      companions: [koban, gameCenter, pachinkoBigSign, flagKoban1],
      boundary: [bar1, bar2, b1, b2, b3, b4],
      access: [mirror1, mirror2, lamp1, pud1, pud2, cone1, cone2],
      livingTrace: garbage,
    });
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    $F(out, 'sign_board', -100, 110); // mahjong 緑看板
    ...
    const lamp3 = $F(out, 'street_lamp', 130, 130);
    ...
    out.humans = [
      _H(-160, 145), _H(-100, 145), _H(-30, 145), _H(60, 145), _H(130, 145),
      _H(-130, 158), _H(-65, 158), _H(0, 158), _H(95, 158),
      _H(-130, 60), _H(130, 60),  // bank/capsule
      _H(-30, 100), _H(40, 100),
    ];
    ...
    _CLUSTER(out, {
      ...
      focal: { kind: 'b', i: 5 },  // bank
      companions: [{ kind: 'f', i: 32 }, { kind: 'f', i: 33 }],
      livingTrace: nwMail,
    });
    _CLUSTER(out, {
      id: 's2.ch5.NE.capsule', role: 'ambient', cell: 'NE',
      focal: { kind: 'b', i: 6 },  // capsule_hotel
      companions: [{ kind: 'f', i: 36 }, { kind: 'f', i: 37 }],
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // ambient
    const club = $B(out, 'club', 130, 22);   // NE
    const snack = $B(out, 'snack', -130, 130); // SW
    const mahjong = $B(out, 'mahjong_parlor', 130, 130); // SE
    $B(out, 'business_hotel', 70, 22);
    $B(out, 'townhouse', 80, 130);
    $B(out, 'townhouse', -50, 130);

    // NW 焦点取り巻き
    const sign = $F(out, 'sign_board', -110, 8); // ピンク大ネオン
    const parasol1 = $F(out, 'parasol', -110, 58);
    const parasol2 = $F(out, 'parasol', -50, 58);
    const flag = $F(out, 'flag_pole', -110, 12);
    // 境界 (高めの hedge、目隠し)
    ...
    const recyc = $F(out, 'recycling_bin', -158, 88);  // NW livingTrace 控えめ
    ...
    out.humans = [
      _H(-110, 60), _H(-50, 60),                     // カップル
      _H(130, 60), _H(150, 80),                      // ホステス
      _H(-130, 145), _H(-100, 158),                  // snack 客
      _H(130, 158),                                   // mahjong
      _H(-30, 100), _H(40, 100),                     // avenue 少
    ];
    ...
    out.grounds = [
      ...
      _G('stone_pavement', -65, 100, 12, 200),
      _G('tile', 130, 60, 60, 40),
    ];

    _CLUSTER(out, {
      id: 's2.ch6.NW.lovehotel', role: 'hero', cell: 'NW',
      focal: loveBig,
      companions: [loveSm, sign, parasol1, parasol2, flag],
      boundary: [h1, h2, h3, h4],
      access: [lamp, pud1, pud2, fb],
      livingTrace: recyc,
    });
    _CLUSTER(out, {
    ...
    _CLUSTER(out, {
      id: 's2.ch6.SW.snack', role: 'ambient', cell: 'SW',
      focal: snack,
      companions: [{ kind: 'f', i: 18 }, { kind: 'f', i: 19 }, { kind: 'f', i: 20 }],
      livingTrace: swGarbage,
    });
    _CLUSTER(out, {
      id: 's2.ch6.SE.mahjong', role: 'ambient', cell: 'SE',
      focal: mahjong,
      companions: [{ kind: 'f', i: 22 }, { kind: 'f', i: 23 }],
      livingTrace: seBike,
    });
  ...
  // ── S2-Ch7: 屋台横丁 (NW+NE merged hero, 横道路ゼロ) ──
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    const out: RawChunkBody = {
      buildings: [], furniture: [], humans: [], grounds: [],
      horizontalRoads: [],   // 屋台中央集約のため横道路ゼロ
      verticalRoads: [_AVE],
    };
    // merged hero: yatai 5 連 (中央)
    const y1 = $B(out, 'yatai', -105, 68);
    const y2 = $B(out, 'yatai', -52, 68);
    const y3 = $B(out, 'yatai', 0, 68);
    const y4 = $B(out, 'yatai', 55, 68);
    const y5 = $B(out, 'yatai', 105, 68);
    // ambient (下段)
    const chaya = $B(out, 'chaya', -100, 130);  // SW
    const ramen = $B(out, 'ramen', 100, 130);   // SE
    $B(out, 'townhouse', -160, 22);
    $B(out, 'townhouse', 160, 22);
    $B(out, 'apartment', -30, 22);

    // 屋台取り巻き
    const c1 = $F(out, 'chouchin', -105, 58);
    const c2 = $F(out, 'chouchin', -52, 58);
    const c3 = $F(out, 'chouchin', 0, 58);
    const c4 = $F(out, 'chouchin', 55, 58);
    const c5 = $F(out, 'chouchin', 102, 58);
    const par1 = $F(out, 'parasol', -105, 78);
    const par2 = $F(out, 'parasol', -52, 78);
    const par3 = $F(out, 'parasol', 0, 78);
    const par4 = $F(out, 'parasol', 55, 78);
    const par5 = $F(out, 'parasol', 102, 78);
    // 客の bench (準対称)
    ...
    const fp2 = $F(out, 'flower_planter_row', 157, 88);
    // 動線
    ...
    const lamp1 = $F(out, 'street_lamp', -90, 88);
    const lamp2 = $F(out, 'street_lamp', 90, 88);
    const garbage = $F(out, 'garbage', -160, 88);

    // SW chaya
    $F(out, 'noren', -100, 122);
    $F(out, 'chouchin', -100, 118);
    const swCat = $F(out, 'cat', -130, 178);
    // SE ramen
    $F(out, 'chouchin', 97, 118); $F(out, 'noren', 97, 122);
    const seBike = $F(out, 'bicycle', 130, 158);
    // 連続軸
    $F(out, 'chouchin', -160, 15); $F(out, 'chouchin', 0, 15);
    $F(out, 'chouchin', 157, 15);
    $F(out, 'puddle_reflection', -45, 105);
    ...
    out.humans = [
      _H(-105, 88), _H(-52, 88), _H(0, 88), _H(55, 88), _H(105, 88),  // 各屋台客
      _H(-78, 80), _H(28, 80), _H(78, 80),                              // 屋台店主
      _H(-100, 152), _H(100, 152),                                      // 茶店/ラーメン
      _H(0, 100),
    ];

    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('red_carpet', 0, 78, 220, 30),     // 屋台床 (祭り感)
      _G('concrete', 0, 100, 280, 30),       // 屋台前
      _G('stone_pavement', -65, 100, 12, 200),
    ];

    _CLUSTER(out, {
      id: 's2.ch7.merged.yatai_yokocho', role: 'hero', cell: 'merged',
      focal: y3,  // 中央屋台
    ...
    _CLUSTER(out, {
      ...
      focal: chaya,
      companions: [{ kind: 'f', i: 26 }, { kind: 'f', i: 27 }],
      livingTrace: swCat,
    });
    _CLUSTER(out, {
      id: 's2.ch7.SE.ramen', role: 'ambient', cell: 'SE',
      focal: ramen,
      companions: [{ kind: 'f', i: 29 }, { kind: 'f', i: 30 }],
      livingTrace: seBike,
    });
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // ambient
    const karaoke = $B(out, 'karaoke', -130, 22);   // NW
    const pachinko = $B(out, 'pachinko', 130, 22);  // NE
    $B(out, 'mansion', -100, 130);                  // SW
    $B(out, 'mansion', -45, 130);
    $B(out, 'business_hotel', -45, 22);
    $B(out, 'shop', 50, 22);

    // SE 焦点取り巻き
    const poster = $F(out, 'sign_board', 130, 118);  // ポスター見立て
    const banner1 = $F(out, 'banner_pole', 100, 120);
    const banner2 = $F(out, 'banner_pole', 158, 120);
    const flag = $F(out, 'flag_pole', 130, 128);
    // 境界
    const fp1 = $F(out, 'flower_planter_row', 110, 170);
    const fp2 = $F(out, 'flower_planter_row', 168, 170);
    const b1 = $F(out, 'bollard', 80, 140);
    const b2 = $F(out, 'bollard', 178, 140);
    // 動線
    const lamp = $F(out, 'street_lamp', 130, 140);
    const benchT1 = $F(out, 'bench', 110, 148);
    const benchT2 = $F(out, 'bench', 152, 148);
    const pud1 = $F(out, 'puddle_reflection', 100, 158);
    const garbage = $F(out, 'garbage', 158, 188);
    ...
    out.humans = [
      _H(130, 145), _H(110, 158), _H(152, 158),  // 映画客
      _H(-130, 60), _H(130, 60),                   // NW/NE 酔客
      _H(-30, 100), _H(40, 100),
    ];
    ...
    out.grounds = [
      ...
      _G('stone_pavement', -65, 100, 12, 200),
      _G('concrete', -75, 152, 110, 30),
    ];

    _CLUSTER(out, {
      id: 's2.ch8.SE.theater', role: 'hero', cell: 'SE',
      focal: theater,
      companions: [poster, banner1, banner2, flag],
      boundary: [fp1, fp2, b1, b2],
      access: [lamp, benchT1, benchT2, pud1],
      livingTrace: garbage,
    });
    _CLUSTER(out, {
      id: 's2.ch8.NW.karaoke', role: 'ambient', cell: 'NW',
      focal: karaoke,
      companions: [{ kind: 'f', i: 18 }, { kind: 'f', i: 19 }],
    ...
    _CLUSTER(out, {
      ...
      focal: { kind: 'b', i: 3 },  // mansion
      companions: [{ kind: 'b', i: 4 }],
      livingTrace: swMail,
    });
  ...
  // ── S2-Ch9: 駐車場と裏通り (NE hero, 静寂転換の入口) ──
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    const out: RawChunkBody = {
      buildings: [], furniture: [], humans: [], grounds: [],
      horizontalRoads: [_MID_HR],
    ...
    // ambient
    $B(out, 'house', -130, 22);   // NW
    $B(out, 'house', -90, 22);
    $B(out, 'kura', -130, 130);   // SW
    $B(out, 'townhouse', 80, 130); // SE
    $B(out, 'townhouse', 140, 130);
    $B(out, 'shed', -160, 130);
    ...
    const bar1 = $F(out, 'barrier', 90, 88);
    const bar2 = $F(out, 'barrier', 170, 88);
    const car1 = $F(out, 'car', 80, 80);
    const car2 = $F(out, 'car', 130, 80);
    const car3 = $F(out, 'car', 170, 80);
    // 境界
    const wfence1 = $F(out, 'wood_fence', 75, 60); $F(out, 'wood_fence', 75, 80);
    const hedge1 = $F(out, 'hedge', 178, 100);
    // 動線
    const lamp = $F(out, 'street_lamp', 130, 95);
    const vending = $F(out, 'vending', 75, 85);  // 孤立感
    const garbage = $F(out, 'garbage', 158, 60);  // NE livingTrace
    ...
    // 連続軸 (Stage 2 終盤)
    $F(out, 'chouchin', -160, 15);  // 1 本だけ残る (寂しさ)
    $F(out, 'street_lamp', -90, 88); $F(out, 'street_lamp', 90, 88);
    $F(out, 'puddle_reflection', -45, 105);
    $F(out, 'puddle_reflection', 0, 100);
    $F(out, 'bollard', -65, 92); $F(out, 'bollard', 62, 92);
    ...
    out.grounds = [
      ...
      _G('stone_pavement', -65, 100, 12, 200),
      _G('concrete', -130, 152, 90, 40),
    ];

    _CLUSTER(out, {
      id: 's2.ch9.NE.parking', role: 'hero', cell: 'NE',
      focal: parking,
      companions: [car1, car2, car3, bar1],
      boundary: [wfence1, hedge1, bar2],
      access: [lamp, cone1, cone3, vending],
      livingTrace: garbage,
    });
    _CLUSTER(out, {
      id: 's2.ch9.NW.house', role: 'ambient', cell: 'NW',
      focal: { kind: 'b', i: 1 },  // house
      companions: [{ kind: 'b', i: 2 }],
      livingTrace: nwMail,
    });
    _CLUSTER(out, {
      id: 's2.ch9.SW.kura', role: 'ambient', cell: 'SW',
      focal: { kind: 'b', i: 3 },  // kura
      companions: [{ kind: 'b', i: 6 }],
      livingTrace: swGarbage,
    });
    _CLUSTER(out, {
      id: 's2.ch9.SE.townhouses', role: 'ambient', cell: 'SE',
      focal: { kind: 'b', i: 4 },  // townhouse
      companions: [{ kind: 'b', i: 5 }],
      livingTrace: seCat,
    });
  ...
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    ...
    // SW hero: 神社の裏 (kura が小社見立て)
    const kura = $B(out, 'kura', -100, 130);
    // ambient
    $B(out, 'kominka', -130, 22);     // NW
    $B(out, 'townhouse', 80, 22);     // NE
    $B(out, 'townhouse', 130, 22);
    $B(out, 'shed', 60, 130);          // SE
    $B(out, 'shed', 130, 130);
    $B(out, 'machiya', -50, 22);
    ...
    const bf1 = $F(out, 'bamboo_fence', -178, 130);
    const bf2 = $F(out, 'bamboo_fence', -178, 170);
    const bf3 = $F(out, 'bamboo_fence', 0, 130);
    const bwf = $F(out, 'bamboo_water_fountain', -70, 158);
    const hedge1 = $F(out, 'hedge', -178, 145);
    // 動線 (参道)
    const swCat1 = $F(out, 'cat', -130, 168);
    ...
    out.grounds = [
      _G('asphalt', 0, 100, 360, 200),
      _G('stone_pavement', -100, 145, 80, 80),     // SW 神社参道
      _G('fallen_leaves', -130, 168, 30, 20),       // 落ち葉
      _G('fallen_leaves', -75, 178, 25, 15),
      _G('stone_pavement', -65, 100, 12, 200),
      _G('concrete', 130, 50, 100, 40),
    ];

    _CLUSTER(out, {
      id: 's2.ch10.SW.shrine_back', role: 'hero', cell: 'SW',
      focal: kura,
      companions: [lant1, lant2, bwf],
      boundary: [bf1, bf2, bf3, hedge1],
      access: [{ kind: 'f', i: 6 }],  // bwf = 動線にも兼用
      livingTrace: swCat1,
    });
    _CLUSTER(out, {
      id: 's2.ch10.NW.kominka', role: 'ambient', cell: 'NW',
      focal: { kind: 'b', i: 1 },  // kominka
      companions: [{ kind: 'f', i: 9 }, { kind: 'f', i: 10 }],
      livingTrace: nwMail,
    });
    _CLUSTER(out, {
      id: 's2.ch10.NE.townhouses', role: 'ambient', cell: 'NE',
      focal: { kind: 'b', i: 2 },  // townhouse
      companions: [{ kind: 'b', i: 3 }],
      livingTrace: neCat,
    });
    _CLUSTER(out, {
      id: 's2.ch10.SE.sheds', role: 'ambient', cell: 'SE',
      focal: { kind: 'b', i: 4 },  // shed
      companions: [{ kind: 'b', i: 5 }],
      livingTrace: seGarbage,
    });
  ...
  // ── S2-Ch11: 神社の表参道 (NE hero, Stage 3 への handoff) ──
  { patternId: 's2_raw', raw: ((): RawChunkBody => {
    const out: RawChunkBody = {
      buildings: [], furniture: [], humans: [], grounds: [],
      horizontalRoads: [_MID_HR],
    ...
    // NE hero: torii + 表参道
    // (注意: torii は家具タイプ。建物は無し、オープンスペース焦点 = 基本形 A)
    $B(out, 'chaya', -100, 22);     // NW (Stage 3 予告)
    const swKura1 = $B(out, 'kura', -130, 130);   // SW
    const swKura2 = $B(out, 'kura', -70, 130);
    $B(out, 'kominka', 130, 130);   // SE
    $B(out, 'shed', -160, 178);
    $B(out, 'shed', 70, 178);
    ...
    const lant1 = $F(out, 'stone_lantern', 40, 88);
    const lant2 = $F(out, 'stone_lantern', 120, 88);
    const koma1 = $F(out, 'koma_inu', 60, 68);
    const koma2 = $F(out, 'koma_inu', 100, 68);
    const rope = $F(out, 'shinto_rope', 80, 22);
    // 境界
    const bf1 = $F(out, 'bamboo_fence', 10, 10);
    const bf2 = $F(out, 'bamboo_fence', 10, 50);
    const bf3 = $F(out, 'bamboo_fence', 10, 90);
    const bf4 = $F(out, 'bamboo_fence', 150, 10);
    const bf5 = $F(out, 'bamboo_fence', 150, 50);
    const bf6 = $F(out, 'bamboo_fence', 150, 90);
    // 動線
    const newsStand = $F(out, 'newspaper_stand', 80, 95);  // NE livingTrace 控えめ

    // NW chaya (Stage 3 予告)
    $F(out, 'noren', -100, 28);
    const nwBike = $F(out, 'bicycle', -125, 60);
    // SW kura
    $F(out, 'wood_fence', -130, 145); $F(out, 'wood_fence', -70, 145);
    const swGarbage = $F(out, 'garbage', -160, 188);
    ...
    _CLUSTER(out, {
      id: 's2.ch11.NE.torii_sando', role: 'hero', cell: 'NE',
      focal: torii,
      companions: [sandoP1, sandoP2, lant1, lant2, koma1],
      boundary: [bf1, bf2, bf3, bf4, bf5, bf6],
      access: [rope, koma2],
      livingTrace: newsStand,
    });
    _CLUSTER(out, {
      ...
      companions: [{ kind: 'f', i: 14 }],  // noren
      livingTrace: nwBike,
    });
    _CLUSTER(out, {
      id: 's2.ch11.SW.kura', role: 'ambient', cell: 'SW',
      focal: swKura1,
      companions: [swKura2],
      livingTrace: swGarbage,
    });
