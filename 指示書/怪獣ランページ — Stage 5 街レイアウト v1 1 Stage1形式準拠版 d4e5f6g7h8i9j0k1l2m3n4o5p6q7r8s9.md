# 怪獣ランページ — Stage 5 街レイアウト v1.1 Stage1形式準拠版
...
## Act II — 広場エリア
**取り巻き 3 パターン**:
- facade 沿い (y=22): `chouchin` × 多、`yatai` 上段
...
**取り巻き 3 パターン**:
- facade 沿い (y=22): `balloon_cluster` × 2 (頂上アクセント x=-15/+15, y=8)、`banner_pole` × 4
...
## Act IV — 祭りクライマックス
**取り巻き 3 パターン**:
- facade 沿い (y=22): `flag_pole` × 2、`balloon_cluster` × 4 (上空)
...
**取り巻き 3 パターン**:
- facade 沿い (y=22): `big_tent` × 4 (準対称)、`balloon_cluster` × 6 (上空不均等)
...
**アンビエント (merged 特例)**: 全 merged のため独立アンビエントセル無し。Ch11 GOAL の予告として 4 セル全てが城方向に向いた集約配置（Stage 1 Ch6 と同じ運用）。
...
---
**取り巻き 3 パターン**:
- facade 沿い (y=22): `yatai` 上空 `chouchin` × 多、`flag_pole` × 4 (城方向指す)
- 焦点囲み: 中央 carousel + big_tent × 2 + matsuri_drum × 2 (Ch11 予告)
- 歩道沿い: `popcorn_cart` × 2、`bench` × 多

**生活の痕跡 / スケール / 向き**:
- 痕跡: 観客 × 多、太鼓奏者 × 2 (予告)、子供 × 多
- スケール: 中 `big_tent` × 2・`carousel`/`yatai` × 多 / 小 `matsuri_drum` × 2・`balloon_cluster` × 4
- 向き: flag_pole は南向き (城方向)、太鼓は中央向き

**Handoff (Ch10 → Ch11 GOAL)**:
- `red_carpet` を Ch10 → Ch11 で全幅化
- `chouchin` 大群が Ch11 で城周囲を囲む形に変化
- `matsuri_drum` × 2 (Ch10 予兆) → Ch11 で 4 個 + 4 個 = 8 個に倍増
- avenue 中央に `stone_pavement` 予告パッチ → Ch11 城敷地へ連続
...
### Ch11: 天守閣決戦 (isGoal: true、GOAL チャンク)

**ヒーロー / アンビエント**: **merged 全マージ = HERO GOAL**（castle 中央配置）/ アンビエント無し（全 4 セルが城周囲の応援機構に飲み込まれる）

**merged ヒーロー 4 層**:

- **焦点**: `castle` (x=0, y=40) ★★ ランドマーク (w=70, h=110、上端 y=40 が天守閣屋根、下端 y=150 が石垣、avenue 中央完全占有)
- **取り巻き**: `balloon_cluster` × 11 (天頂 y=4-14、不揃い、x=-150/-110/-75/-40/-15/+10/+35/+70/+100/+130/+155)、`banner_pole` × 5 (x=-100/-50/0/+50/+100, y=18 旗ガーランド)、`flag_pole` × 3 (x=-30/0/+30, y=12)、応援メリー `carousel` × 2 (x=±117, y=68)、観客テント `big_tent` × 2 (x=±140, y=130)、応援太鼓 `matsuri_drum` × 4 (中段 x=±25 y=165、最背面 x=0 y=132/175)、城前屋台 `yatai` × 8 (x=-170/-100/-60/-30/+30/+60/+100/+170, y=132-178 微不揃い)
- **境界**: `sakura_tree` × 2 (準対称、x=±30, y=88) ★ Stage 5 唯一の桜、`flower_bed` × 4 (x=-100/-50/+50/+100, y=170)、`flower_planter_row` × 4 (x=±150/±50, y=88/170)
- **動線**: `ticket_booth` × 2 (城ゲート、x=±85, y=128)、`popcorn_cart` × 2 (x=±110, y=148)、`parasol` × 2 (x=±50, y=170)、`street_lamp` × 2 (x=±65, y=88 参道照明)、**赤絨毯帯** `red_carpet` 全面 (x=0, y=130, 360×140) ★「王座の間」感

**アンビエント 3 セル**: GOAL チャンクは全 merged の特例。アンビエントは設定せず、城を取り囲む応援機構が 4 セル全てを占める（Stage 1 Ch6 全マージと同じ運用）。

**取り巻き 3 パターン**:

- **facade 沿い** (y=22): `banner_pole` × 5 (x=-100/-50/0/+50/+100, y=18) 旗ガーランド帯、`flag_pole` × 3 (x=-30/0/+30, y=12)
- **焦点囲み**: castle 周辺の `carousel` × 2 + `big_tent` × 2 + `matsuri_drum` × 4 + `yatai` × 8 + `bench` × 6（観客溜まり）
- **歩道沿い**: `street_lamp` × 2 (x=±65, y=88)、`popcorn_cart` × 2、`parasol` × 2、`ticket_booth` × 2 城ゲート
...
**道路**: `verticalRoads: [_AVE]`（城の威光を活かす、追加なし）/ `horizontalRoads: []`（横道路ゼロ、城へ参道が一直線）

**生活の痕跡 / スケール / 向き**:

- **痕跡**: 観客 × 多 (城前応援団・太鼓奏者・屋台客)、儀式の中心 humans × 12
- **スケール**: 大 `castle` ★★ / 中 `carousel` × 2・`big_tent` × 2・`yatai` × 8 / 小 `balloon_cluster` × 11・`matsuri_drum` × 4・`bench` × 6
- **向き**: 全要素が castle 中央向き、太鼓は儀式中心向き、桜並木は参道向き
